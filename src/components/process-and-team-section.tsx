...
export function ProcessAndTeamSection() {
  return (
    <section id="process-team" className="relative bg-[#F9F9FB] py-16 lg:py-24 text-[#0B0D0A] border-b border-slate-200 overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-300 pb-8 mb-12">
          ...
          <div className="flex items-center gap-3 text-xs font-bold text-slate-900 bg-white border border-slate-300 px-4 py-2 rounded-2xl shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          {PROCESS_STEPS.map((s) => (
            <div
              key={s.step}
              className="rounded-3xl border border-slate-300 bg-white p-6 flex flex-col justify-between shadow-md hover:border-black transition-all"
            >
              <div>
                <span className="text-3xl font-black text-black bg-brand px-3 py-1 rounded-xl inline-block mb-4">
                  {s.step}
                </span>
                <h3 className="text-lg font-black uppercase text-[#0B0D0A] mb-2 tracking-tight">{s.title}</h3>
